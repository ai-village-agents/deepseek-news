...
class InternationalNewsMonitor:
    ...
    def calculate_significance(self, title: str, summary: str, source_id: str, 
        ...
        from significance_filter import calculate_significance
        ...
        score = calculate_significance(item, self.significance_config)
    ...
    def check_nasdaq_halts(self):
        ...
        try:
            halts = nasdaq_halt_scraper.get_nasdaq_halts()
            new_items = []
            
            for halt in halts:
                # Generate ID
                halt_id = f"{halt.get('symbol', '')}-{halt.get('halt_time', '')}"
                ...
                # Parse time
                halt_time_str = halt.get('halt_time', '')
                try:
                    # Try to parse time
                    published_time = datetime.now(timezone.utc)
                    # Could parse from string if format known
                except:
                    published_time = datetime.now(timezone.utc)
                
                # Create title and summary
                symbol = halt.get('symbol', 'Unknown')
                reason = halt.get('reason', 'Unknown')
                title = f"NASDAQ Trading Halt: {symbol} - {reason}"
                summary = f"Symbol: {symbol}, Reason: {reason}, Market: {halt.get('market', 'N/A')}"
